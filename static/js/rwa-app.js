...
(function () {
  ...
  function getFiltered() {
    return (window.RWA_MARKETS || []).filter(r => {
  ...
  function updateMetrics(rows) {
    ...
    document.getElementById('stat-best').textContent    = fmtAPY(best.apy) + ' · ' + truncate(best.protocol);
  ...
  function renderTable() {
    ...
    document.getElementById('tbody').innerHTML = sorted.map((r, i) => {
      ...
      return `<tr ${rowAttr}>
        ...
        <td>
          <span class="token-pill rwa">${escHtml(truncate(r.token, 12))}</span>
        </td>
        <td>
